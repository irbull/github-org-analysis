        ...
        promises.push(new Promise((resolve, reject) => {
            github.repos.getCommit({ 'owner': settings.organization, 'repo': repo.name, 'sha': repo.default_branch }, ((err, commit) => {
                ...
                if ( commit && commit.data && commit.data.author) {
                  resolve(`${repo.name}, ${repo.full_name}, ${repo.html_url}, ${repo.updated_at}, ${repo.default_branch}, ${commit.data.author.login}`)
                } else if ( commit && commit.data && commit.data.committer) {
                  resolve(`${repo.name}, ${repo.full_name}, ${repo.html_url}, ${repo.updated_at}, ${repo.default_branch}, ${commit.data.committer.login}`)
                } else if ( commit && commit.data && commit.data.commit && commit.data.commit.committer ) {
                  resolve(`${repo.name}, ${repo.full_name}, ${repo.html_url}, ${repo.updated_at}, ${repo.default_branch}, ${commit.data.commit.committer.email}`)
                } else {
                  resolve(`${repo.name}, ${repo.full_name}, ${repo.html_url}, ${repo.updated_at}, ${repo.default_branch}, undefined`)
                }
            }));
        }));
    }
